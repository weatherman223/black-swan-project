...
def calculate_annual_max(ds_point: xr.Dataset, hist_point: xr.Dataset):
    ...
    rolling_7d = daily.rolling(time=7, center=True).sum()  # 7‑day totals
    ...
    hist_roll = hist_daily.rolling(time=7, center=True).sum()
...
def main():
    ...
    with PdfPages(output_pdf) as pdf:
        ...
        ax.scatter(all_years, all_maxima, color='black', s=1, alpha=0.5, label='Annual Maxima')
        ax.axhline(threshold, color="red", ls="--", label=f"Threshold (4σ = {threshold:.3f} m)")
        ax.axhline(extreme_threshold, color="purple", ls="--", label=f"Extreme Threshold (10σ = {extreme_threshold:.3f} m)")
        ax.axhline(mean, color="blue", ls="--", label=f"Historical Mean = {mean:.3f} m")
        # Plot exceedances as larger, highlighted dots
        ax.scatter(all_exceedance_years, all_exceedance_values, s=25, color='orange', zorder=5, label=f'Exceedances ({len(all_exceedance_values)} total)')
        ax.scatter(all_extreme_exceedance_years, all_extreme_exceedance_values, s=25, color='red', zorder=5, label=f'Extreme Exceedances ({len(all_extreme_exceedance_values)} total)')

        ax.set_xlabel("Year")
        ax.set_ylabel("Precipitation (m)")
        ax.set_ylim(0, 1.3)
        ax.set_title(f"All Ensemble Members (Historical) – Kerr County, TX (7d rolling window maxima)\n{len(patterns)} Ensembles Combined")
        ax.legend(fontsize="small")
        ax.grid(True, alpha=0.3)

        plt.tight_layout()
        plt.savefig("/yourdirectoryhere/result.pdf", dpi=300, bbox_inches='tight')
        print("Combined plot saved to all_ensembles_combined.png and .pdf")
        plt.close()
